...
Obj *make_function(Obj *, Obj *, Obj *, int);
...
Obj *prim_defmacro(Obj *env, Obj *args)
{
	Obj *sym = args->car;
	Obj *params = args->cdr->car;
	Obj *body = args->cdr->cdr->car;
	Obj *fn = make_function(params, body, env, T_MACRO);
	add_var(env, sym, fn);
	return fn;
}

Obj *prim_macro_expand(Obj *env, Obj *args)
{
	Obj *fn = eval(env, args->car->car);
	Obj *params = fn->params;
	Obj *body = fn->body;
	Obj *new_env = push_env(fn->env, params, args->car->cdr);
	Obj *new_exp = eval(new_env, body);
	return new_exp;
}

Obj *prim_if(Obj *env, Obj *args)
{
	Obj *if_part = eval(env, args->car);
	Obj *then_part = eval(env, args->cdr->car);
	Obj *else_part = eval(env, args->cdr->cdr->car);
	if (if_part->value != 0)
		return then_part;
	else
		return else_part;
...
Obj *prim_lambda(Obj *env, Obj *args)
{
	Obj *params = args->car;
	Obj *body = args->cdr;
	return make_function(params, body, env, T_FUNCTION);
}
...
Obj *make_function(Obj *params, Obj *body, Obj *env, int type)
{
	Obj *o = malloc(sizeof(Obj));
	o->params = params;
	o->body = body;
...
Obj *apply(Obj *env, Obj *fn, Obj *args)
{
	...
	else
	{
		Obj *params = fn->params;
		Obj *body = fn->body;
		Obj *new_env = push_env(fn->env, params, args);
		Obj *new_exp = eval(new_env, body);
		return eval(env, new_exp);
	}
}
...
void define_prim_function(Obj *env)
{
	...
	add_prim(env, "if", prim_if);
	add_prim(env, "defmacro", prim_defmacro);
	add_prim(env, "macroexpand", prim_macro_expand);
}
int main()
{
	//给一些单例的常量赋值
